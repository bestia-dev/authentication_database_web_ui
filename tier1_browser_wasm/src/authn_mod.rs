...
pub mod authn_login {
    ...
    pub async fn btn_submit() {
        ...
        if !user_email.is_empty() && !password.is_empty() {
            ...
            let resp_obj: common_code::DataRespAuthnLoginProcessEmail = fetch_object_response(
            ...
            .await;
            debug_write(&format!("response salt: {}", resp_obj.salt));
            ...
            .await;
            login_success = resp_obj.login_success;
        ...
        if login_success == true {
            window()
                .open_with_url(&format!("/{APP_MAIN_ROUTE}/webpage_hits/webpage_hits_list"))
                .unwrap();
        } else {
            let div_alert = get_html_element_by_id("div_alert");
            div_alert.set_text_content(Some("Authentication failed !"));
            // TODO: on key_up for email or password empty the alert
        }
    }
}
